...
def _add_loss_summaries(total_loss):
    ...
    Generates loss summaries for visualising the performance of the net.
...
class ModelSkeleton:
    ...
    def __init__(self, mc):
        ...
        # a scalar tensor in range (0, 1].
        # Usually set to 0.5 in training phase and 1.0 in evaluation phase
        ...
        self.image_input, self.input_mask, self.box_delta_input, self.box_input, self.labels = tf.compat.v1.train.batch(
    ...
    def _add_interpretation_graph(self):
        ...
        with tf.compat.v1.variable_scope('IOU'):
            def _tensor_iou(box1, box2):
                ...
                return intersection / (union + mc.EPSILON) * tf.reshape(self.input_mask, [mc.BATCH_SIZE, mc.ANCHORS])
    ...
    def _add_viz_graph(self):
        ...
        self.viz_op = tf.summary.image('sample_detection_results',
                                       self.image_to_show, max_outputs=mc.BATCH_SIZE)
    ...
    def filter_prediction(self, boxes, probs, cls_idx):
        ...
        if len(probs) > mc.TOP_N_DETECTION > 0:
