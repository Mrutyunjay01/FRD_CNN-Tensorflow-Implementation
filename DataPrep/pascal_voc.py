...
from DataPrep.imdb_wrapper import imdb
from DataPrep.pascal_voc_eval import voc_eval
    ...
    def _load_pascal_annotation(self):
        ...
        for index in self._image_idx:
            ...
            for obj in objs:
                ...
                assert 0.0 <= xmin <= xmax, \
                    'Invalid bounding box x-coord xmin {} or xmax {} at {}.xml'.format(xmin, xmax, index)
                assert 0.0 <= ymin <= ymax, \
                    'Invalid bounding box y-coord ymin {} or ymax {} at {}.xml'.format(ymin, ymax, index)
